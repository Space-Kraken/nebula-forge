...
| `--media <bucket>` | Sirve un bucket del mismo módulo detrás del nuevo `static-site` en `/media/*` (S3+OAC, cacheado, solo GET; se pregunta si hay buckets) |
...
## Exportar el modelo (integración con plataformas)

### `forge model --json [--env <e>]`
Emite a stdout un JSON **versionado** (`schemaVersion: 1`) con el contrato del
workspace para tooling externo (generadores de policies IAM, validadores):
por entorno, dominios con stack y **tags resueltos** (heredados incluidos), y
por componente su **nombre físico renderizado** — o el marcador literal
`"autogenerated"` donde forge no nombra a propósito — más el **manifiesto de
capacidades** (tipos CloudFormation/Terraform que su builder puede emitir).
Offline, sin credenciales. Los warnings van a stderr; stdout es JSON puro.
Política de versionado: cambios aditivos no suben `schemaVersion`; renombres
o cambios de semántica sí.
