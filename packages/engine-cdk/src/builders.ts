...
/**
 * Deterministic physical name for DECORATIVE resources only (rest apis,
 * tables, user pools — addressed by id/Ref, never by name): omitted when it
 * would exceed the service limit, letting CloudFormation autogenerate. That
 * fallback is DECLARED in the exported model as "autogenerated". Resources
 * forge addresses by name (functions, event buses) never use this — the
 * loader rejects over-long names for them at load time.
 */
function decorativeName(ctx: BuildContext, componentName: string, maxLength: number): string | undefined {
...
function createFunction(
  ...
  return new NodejsFunction(scope, id, {
    // Always set: the loader guarantees ≤64 chars, and gateway mounts build
    // ARNs from this exact name — a silent fallback would break addressing.
    functionName: resourceNameFor(ctx.model.name, ctx.domain.name, spec.name, ctx.environment),
  ...
  switch (spec.type) {
    ...
    case 'http-api': {
      ...
      const api = new apigateway.RestApi(scope, `${id}RestApi`, {
        restApiName: decorativeName(ctx, spec.name, 128),
    ...
    case 'table': {
      const table = new dynamodb.Table(scope, `${id}Table`, {
        tableName: decorativeName(ctx, spec.name, 255),
        partitionKey: {
    ...
    case 'auth': {
      const pool = new cognito.UserPool(scope, `${id}UserPool`, {
        userPoolName: decorativeName(ctx, spec.name, 128),
