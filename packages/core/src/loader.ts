  ...
  for (const domain of model.domains) {
    for (const component of domain.components) {
      ...
      if (component.config.mount) {
        ...
        // Physical function-name length (the gateway integrates this Lambda
        // by deterministic ARN) is enforced for EVERY function-like
        // component in validatePhysicalNames — no mount-specific check needed.
...
/**
 * Physical-name length limits for the resources forge ADDRESSES by
 * deterministic name. Everything else (rest api, table, user pool names) is
 * decorative — addressed by id/Ref, so an over-long name falls back to a
 * CloudFormation-generated one and is DECLARED "autogenerated" in the
 * exported model instead of failing.
 *   aws: Lambda functionName (gateway mounts build ARNs from it) = 64,
 *        EventBridge bus (cross-domain publish/subscribe by name) = 256.
 *   azure: Event Grid topic (cross-domain data sources rebuild it) = 50;
 *          function apps go through the hashed globalName helper and can
 *          never exceed their limit.
 */
function physicalLimitFor(engine: string, type: string): { limit: number; service: string } | undefined {
  if (engine === 'azure-terraform') {
    return type === 'event-bus' ? { limit: 50, service: 'Event Grid topic' } : undefined;
  }
  if ((FUNCTION_LIKE_TYPES as readonly string[]).includes(type)) {
    return { limit: 64, service: 'Lambda' };
  }
  return type === 'event-bus' ? { limit: 256, service: 'EventBridge bus' } : undefined;
}
 ...
 * a name that cannot deploy (or that cross-domain addressing could not
 * reproduce) must fail here, not in CDK or Terraform.
 */
  ...
  for (const envName of Object.keys(model.environments)) {
    ...
    for (const domain of model.domains) {
      for (const component of [...domain.components, ...(domain.packComponents ?? [])]) {
        ...
        const physical = physicalLimitFor(model.engine, component.type);
        if (physical && rendered.length > physical.limit) {
          throw new ForgeError(
            `Component "${domain.name}/${component.name}" renders the physical name "${rendered}" (${rendered.length} chars) in environment "${envName}", over the ${physical.limit}-character ${physical.service.includes('Lambda') ? 'function name limit' : `${physical.service} name limit`}`,
            'Shorten the component, module or project name — or forge.json naming.pattern if one is configured. This name is addressed deterministically, so it can never fall back to an autogenerated one.',
          );
        }
      }
    }
