...
[![npm](https://img.shields.io/npm/v/%40space-kraken%2Fnebula-forge?label=%40space-kraken%2Fnebula-forge)](https://www.npmjs.com/package/@space-kraken/nebula-forge)
[![provenance](https://img.shields.io/badge/npm-provenance-brightgreen)](https://www.npmjs.com/package/@space-kraken/nebula-forge)
...
- **Inherited conventions**: `"conventions"` in forge.json points at an
  org-owned npm package exporting `{ naming, tags, environments }` — the
  rule lives once; deviating requires an explicit `overrides` block that
  loads with a warning. Bumping the package version is a migration event.
  The package may export a function of the workspace context instead: an
  **adapter** that maps the org's own standards (its vocabulary, its config
  files) onto forge's small, versioned contract — forge never learns the
  org's format (see `examples/forge-conventions-adapter/`).
  `forge new --conventions <pkg>` scaffolds a workspace already adhered.
